...
/** @private  */
const formatProfile$1 = (profile) => String(profile).padStart(2, "0");

/** @private  */
const formatLevel$3 = (level) => String(parseFloat(level) * 10).padStart(2, "0");

/** @private  */
const formatBitDepth$1 = (bitDepth) => String(bitDepth).padStart(2, "0");

/** @private  */
const formatCodec$3 = (cccc, PP, LL, DD) => `${cccc}.${PP}.${LL}.${DD}`;
    ...
    VP_PROFILES.map((profile) =>
      VP_LEVELS.map((level) =>
        VP_BIT_DEPTH.map((bitDepth) => ({
          ...
          codec: formatCodec$3(
            codec.cccc,
            formatProfile$1(profile),
            formatLevel$3(level),
            formatBitDepth$1(bitDepth),
          ),
        })),
      ),
    ),
...
const getCodecName$3 = (codec) =>
  getAllItems$3().find((item) => item.codec === codec)?.name;
...
/** @private  */
const convertLevel$1 = (level) => {
  const [X, Y] = level.split(".");
  return (parseInt(X, 10) - 2) * 4 + parseInt(Y, 10);
};

/** @private  */
const formatLevel$2 = (level) => String(convertLevel$1(level)).padStart(2, "0");

/** @private  */
const formatTier$1 = (tier) => tier.at(0);

/** @private  */
const formatBitDepth = (bitDepth) => String(bitDepth).padStart(2, "0");

/** @private  */
const formatCodec$2 = (cccc, P, LL, T, DD) => `${cccc}.${P}.${LL}${T}.${DD}`;
...
const getCodecName$2 = (codec) =>
  getAllItems$2().find((item) => item.codec === codec)?.name;
...
/** @private */
const formatLevel$1 = (level) =>
  (parseFloat(level) * 10).toString(16).padStart(2, "0");

/** @private */
const formatCodec$1 = (cccc, { PP, CC }, LL) => `${cccc}.${PP}${CC}${LL}`;
...
const getCodecName$1 = (codec) =>
  getAllItems$1().find((item) => item.codec === codec)?.name;
